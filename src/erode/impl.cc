...
std::vector<cv::Mat> erode(const cv::Mat& src_erode, const cv::Mat& src_dilate) {
    ...
    Mat kernal1 = getStructuringElement(MORPH_RECT, Size(7,7));
    erode(binary_erode, dst_erode, kernal1, Point(-1,-1), 1);
    
    Mat gray_dilate;
    Mat binary_dilate;
    cvtColor(src_dilate, gray_dilate, COLOR_BGR2GRAY);
    threshold(gray_dilate, binary_dilate, 50, 255, THRESH_BINARY);
    Mat kernal2 = getStructuringElement(MORPH_RECT, Size(13,13));
    dilate(binary_dilate, dst_dilate, kernal2, Point(-1,-1), 1);
